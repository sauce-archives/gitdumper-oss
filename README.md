...
> Pick up on github links, and post pretty things to slack.

This leverages [python-rtmbot](https://github.com/slackhq/python-rtmbot) to connect to slack using a bot integration.
The bot integration exists in Saucelabs' Slack Integrations section. Its API token is housed in `rtmbot.conf` in the
root of this repo.

# Quickstart / Requirements

    mkvirtualenv gitdumper
    pip install -r requirements.txt
    ./rtmbot.py # this runs the bot

# Hacking Away

gitdumper picks up on messages in the Python module located in `plugins/github/github.py`.

Incoming messages are picked up in the `process_message` method.

Outgoing messages are appended to the `output` objects within.

# Deployment
get in your virtual environment, then run
```
./rtmbot.py
```
## Contributors
...
# python-rtmbot documentation below!
